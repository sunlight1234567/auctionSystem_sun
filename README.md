...
*   **用户角色**：
    ...
    *   **卖家**：发布拍品（支持 Decimal 高精度定价）、管理草稿、查看销售记录、处理物流、对被驳回商品发起申诉。
    *   **买家**：浏览搜索、参与竞拍、中标支付、即时通讯。
...
### 典型测试流程
1.  **管理员审核**：登录 `admin`，进入后台管理页面（Dashboard），在 **Audit** 标签页查看待审核列表。
2.  **卖家发布**：注册账户（角色选卖家），发布一个新拍品（价格系统已升级为高精度 Decimal，防止精度丢失）。
3.  **商品审核与申诉**：
    *   管理员驳回某个申请。
    *   卖家在“我的拍卖”中查看被拒理由，点击“申诉”按钮提交申诉。
    *   管理员在后台 **Appeals** 标签页处理申诉请求。
